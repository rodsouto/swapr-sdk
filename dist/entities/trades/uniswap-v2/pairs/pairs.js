 ...
 */
async function getAllCommonUniswapV2Pairs({ currencyA, currencyB, platform, provider, }) {
    // Return value
    const pairList = [];
    ...
    const bases = constants_2.BASES_TO_CHECK_TRADES_AGAINST[chainId] ?? [];
    ...
    const getReservesAndSwapFeeCallResults = (await multicallContract.callStatic.tryAggregate(false, multicall2CallData));
 ...
 *
 */
async function getAllCommonUniswapV2PairsFromSubgraph({ currencyA, currencyB, platform, }) {
    const chainId = currencyA.chainId ?? currencyB.chainId;
    ...
    const results = await (0, graphql_1.getSdk)(new graphql_request_1.GraphQLClient(subgraphEndpoint)).GetAllCommonPairsBetweenTokenAAndTokenB({
    ...
    const pairSwapFeeList = await (0, fees_1.getUniswapV2PairSwapFee)({

...
class SwaprV3Trade extends trade_1.TradeWithSwapTransaction {
    ...
    static async getQuote({ amount, quoteCurrency, tradeType, maximumSlippage }, provider, isSingleHop) {
        ...
        maximumSlippage = maximumSlippage ?? constants_2.maximumSlippage;
        provider = provider ?? (0, utils_1.getProvider)(chainId);
        (0, tiny_invariant_1.default)((await provider.getNetwork()).chainId == chainId, `SwaprV3Trade.getQuote: currencies chainId does not match provider's chainId`);
        ...
        if (isSingleHop) {
            if (isTradeExactInput) {
                const quotedAmountOut = await (0, contracts_1.getQuoterContract)()
                ...
                const quotedAmountIn = await (0, contracts_1.getQuoterContract)()
        ...
        const routes = isTradeExactInput
            ? await (0, routes_1.getRoutes)(setToken, quoteToken, chainId)
            : await (0, routes_1.getRoutes)(quoteToken, setToken, chainId);
        ...
        const quotesResults = await (0, singleContractMultipleData_1.singleContractMultipleData)(methodName, quoteParams, {
        ...
        const fee = routes?.length > 0 && routes[0].pools.length > 0
    ...
    async swapTransaction(options) {
        ...
        const populatedTransaction = await (0, contracts_1.getRouterContract)().populateTransaction[methodName](params, { value });
        return populatedTransaction;
    }
    async multiSwapTransaction(options) {
        const isNativeIn = currency_1.Currency.isNative(this.inputAmount.currency);
        const recipient = (0, sdk_core_1.validateAndParseAddress)(options.recipient);
        const amountIn = `0x${this.maximumAmountIn().raw.toString(16)}`;
        const amountOut = `0x${this.minimumAmountOut().raw.toString(16)}`;

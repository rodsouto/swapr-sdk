...
class ZeroXTrade extends trade_1.TradeWithSwapTransaction {
    ...
    callData;
    value;
    constructor({ breakdown, input, output, maximumSlippage, tradeType, to, callData, value, priceImpact, estimatedGas, }) {
    ...
    static async bestTradeExactIn(currencyAmountIn, currencyOut, maximumSlippage) {
        ...
        try {
            ...
            const response = await (0, node_fetch_1.default)(apiUrlParams, { headers });
            if (!response.ok)
            ...
            bestTrade = new ZeroXTrade({
                ...
                estimatedGas: bignumber_1.BigNumber.from(json.estimatedGas),
            });
        }
        catch (error) {
    ...
    static async bestTradeExactOut(currencyIn, currencyAmountOut, maximumSlippage) {
        ...
        let bestTrade;
        try {
            const sellToken = currency_1.Currency.isNative(currencyIn) ? currencyIn.symbol : tokenIn.address;
            const buyToken = currency_1.Currency.isNative(currencyAmountOut.currency)
            ...
            bestTrade = new ZeroXTrade({
                ...
                input: currency_1.Currency.isNative(currencyIn)
                    ? currencyAmount_1.CurrencyAmount.nativeCurrency(json.sellAmount, chainId)
                    : new tokenAmount_1.TokenAmount(tokenIn, json.sellAmount),
